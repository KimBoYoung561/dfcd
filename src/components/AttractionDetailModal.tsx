      ...
      <div className="relative flex max-h-[90vh] w-full max-w-lg flex-col rounded-t-[28px] sm:rounded-3xl bg-white text-slate-900 shadow-2xl overflow-hidden border border-slate-200 animate-in slide-in-from-bottom duration-300">
        ...
        <div className="relative h-48 sm:h-56 w-full shrink-0 bg-slate-800 overflow-hidden">
          ...
          <button
            type="button"
            onClick={onClose}
            className="absolute right-3.5 top-3.5 z-20 flex h-9 w-9 min-w-[36px] min-h-[36px] items-center justify-center rounded-full bg-black/50 text-white hover:bg-black/70 active:scale-95 transition-all"
            aria-label="닫기"
          >
            <X size={18} />
          </button>

          {/* Official Image Attribution (우측 하단 출처 표기) */}
          <div className="absolute bottom-3 right-3.5 z-20 rounded-md bg-black/70 backdrop-blur-md px-2 py-0.5 text-[10px] font-medium text-white/95 border border-white/20 shadow-xs">
            출처: {spot.imageSource || '안양시 문화관광'}
          </div>

          {/* Badge & Title */}
          <div className="absolute bottom-3.5 left-4 right-28 text-white z-10">
            ...
            <h2 className="text-xl sm:text-2xl font-black tracking-tight leading-tight drop-shadow-md truncate">
              {spot.name}
            </h2>
          </div>
        </div>
